...
#     print(i)
#     for func in [method2, method3]:
#         start = time.time()
#         print(func(sample), end=' ')
...
#
#     bot_keys = []
#     bot_keys.append(set(''.join(line[:y] for line in cave_map[:x])) & set(key_chars))
#     bot_keys.append(set(''.join(line[y:] for line in cave_map[:x])) & set(key_chars))
#     bot_keys.append(set(''.join(line[:y] for line in cave_map[x:])) & set(key_chars))
#     bot_keys.append(set(''.join(line[y:] for line in cave_map[x:])) & set(key_chars))
#     # print('\n'.join(cave_map))
...
#     initial_state = bots, frozenset()
#     states = {
#         initial_state: 0
#     }
#     paths = deque()
#     paths.append(initial_state)
#     bot_done = [False,False,False,False]
#     while paths:
#         bots, keys_collected = state = paths.popleft()
#         for bot_number, bot in enumerate(bots):
#             botx, boty = bot
#             if keys_collected >= bot_keys[bot_number]:
#                 if not bot_done[bot_number]:
#                     bot_done[bot_number] = True
#                     print(f'Bot {bot_number} done')
#                 continue
...
#                 else:
#                     new_keys_collected = keys_collected
#                 new_bots = bots[:bot_number] + ((xa, ya),) + bots[bot_number + 1:]
#                 new_state = (new_bots, new_keys_collected)
#                 if new_state not in states:
#                     states[new_state] = states[state] + 1
#                     paths.append(new_state)
#     return min([v for k, v in states.items() if len(k[-1]) == key_count])
...
#                 if value == WALL:
#                     continue
#                 if value in door_chars:
#                     keys_needed = keys_needed | {value}
#                 new_state = (xa, ya), keys_needed
#                 if new_state not in states:
#                     if value in key_chars:
#                         nodes.add(new_state)
#                     states[new_state] = states[state] + 1
#                     to_visit.append(new_state)
#         return {k:v for k,v in states.items() if k in nodes}
#
#     x, y = [(row, col) for row, line in enumerate(cave_map) for col, ch in enumerate(line) if ch == '@'][0]
#     cave_map[x - 1] = cave_map[x - 1][:y - 1] + '@#@' + cave_map[x - 1][y + 2:]
#     cave_map[x] = cave_map[x][:y - 1] + '###' + cave_map[x][y + 2:]
...
#
#     print('\n'.join(cave_map))
#     bot_nodes = [scan(bot) for bot in bots]
#     pprint(bot_nodes)
...
# for maze in [sample1, sample2, sample4]:
